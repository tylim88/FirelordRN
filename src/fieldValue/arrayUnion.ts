...
/**
Returns a special value that can be used with @@react-native-firebase/firestore/lite#(setDoc:1) or * @@react-native-firebase/firestore/lite#(updateDoc:1) that tells the server to union the given elements with any array value that already exists on the server. Each specified element that doesn't already exist in the array will be added to the end. If the field being modified is not already an array it will be overwritten with an array containing exactly the specified elements.
...
export const arrayUnion = <Elements extends unknown[]>(
	...
	const ref = getFirestore.FieldValue.arrayUnion(
		...elements
